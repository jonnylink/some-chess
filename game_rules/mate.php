...
function legalMove($possLocations,$possSq,$currSq,$possPiece){
	if($possPiece{0} == 'q'){include_once('game_rules/queen.php'); 	$legal 	= queenRules($possLocations,$possSq,$currSq,$possPiece);}
	if($possPiece{0} == 'r'){include_once('game_rules/rook.php');	$legal 	= rookRules($possLocations,$possSq,$currSq,$possPiece);}
	if($possPiece{0} == 'b'){include_once('game_rules/bishop.php'); 	$legal 	= bishopRules($possLocations,$possSq,$currSq,$possPiece);}
	if($possPiece{0} == 'n'){include_once('game_rules/knight.php'); 	$legal 	= knightRules($possLocations,$possSq,$currSq,$possPiece);}
...
//-- can the threat be captured
	...
	for($x=0;$x<$capNum;++$x){
		...
		if(!$mate && legalMove($possLocations,$check[0],$canCapture[$x],$possPiece)) $legalCap = true;
	}	
			...
			if($check[0]{0} == $kingSq{0}){
				...
				//--check up				
					...
					for($y=1;$y<$num;++$y){
						...
						$blocker 	= aThreat($threatSq,$newLocations,$kingPiece{1},false);						
		...
		if($checkPiece{0} == 'b' || $checkPiece{0} == 'q'){
			...
			//--up & right
				...
				for($y=1;$y<=$num;++$y){
					...
					if($blocker || $pawnBlocker){ $blocked[] = $threatSq; break; }
				}
