...
function login($userName,$password,$loginStr,$add_reg_class){
	$userName	= validate($userName);
	$password	= validate($password);
	if(!$userName || !$password){ 	//--are both fields filled in
		return'<div class="error"'.$add_reg_class.'>'.$loginStr[3].'</div>';
	}else{
		...
		if($numUsers > 1){			//--if account name isn't unique
			return'<div class="error'.$add_reg_class.'">'.$loginStr[4].'</div>';
		}elseif($numPlayers == 0){ 	//--if username isn't found
			return'<div class="error'.$add_reg_class.'">'.$loginStr[5].'</div>';
...
function requestPass($userName,$answer,$loginStr,$add_reg_class){
	include_once('standard.php');
	$userName	= validate($userName);
	...
	if(!$userName || !$answer){ 	//--are both fields filled in
		...
		if($numUsers > 1){			//--if account name isn't unique
			...
			return '<div class="message'.$add_reg_class.'">'.$loginStr[11].$newPass.'</div>';
		}
	}
}
?>